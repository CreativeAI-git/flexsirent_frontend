...
import { fetchUserInquiries } from "../../../redux/features/user/actions/inboxAction";
...
const InquiryManagement = () => {
  ...
  const { inquiryHeader } = useTableHeaders();
  const user = { name: t("sidebar.inquiry_management"), role: "guestBusiness" };
  const [searchFilter, setSearchFilter] = useState("");
  const debouncedSearch = useDebounce(searchFilter, 500);
  const [isViewModal, setIsViewModal] = useState(false);
  const [selectedData, setSelectedData] = useState({});

  const { inquiryList, isLoading } = useSelector(
  ...
  const filteredList =
    inquiryList?.filter((item) => {
      const searchText =
        `${item?.name} ${item?.property_title || ""} ${item?.message || ""} ${item?.email || ""}`.toLowerCase();
