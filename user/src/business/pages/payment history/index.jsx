...
import PaymentHistoryList from "../../../user/components/Tables/PaymentHistoryList";
...
const PaymentHistory = () => {
  ...
  const { businessPaymentsHeader } = useTableHeaders();
  const [searchFilter, setSearchFilter] = useState("");
  const { payHisList, isLoading } = useSelector(
    (state) => state.guest.inbox
  );

  const user = { name: t("sidebar.payment_history"), role: "guestBusiness" };
      ...
      <div className="row">
        <div className="col-md-12 mt-4">
          <div className="d-flex align-items-center justify-content-between gap-3 mb-3 ct_flex_col_575">
            <h4 className="ct_fs_20 ct_fw_600 mb-0">{t("table.payment_information")}</h4>
          ...
          <div className="mt-4">
            <PaymentHistoryList
              data={filteredData}
              tableHeading={businessPaymentsHeader}
            />
          </div>
        </div>
      </div>
