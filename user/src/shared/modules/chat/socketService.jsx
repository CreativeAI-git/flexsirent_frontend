...
import { BASE_URL ,SOCKET_URL} from "../../routes/apiURLs";
...
export const connectScopeSocket = (scope, options = {}) => {
  ...
  socket.on("connected_user", (user) => {
    if (typeof onConnectedUser === "function") {
      onConnectedUser(user);
    }
  });

  socket.on("chat_list", (list) => {
    if (typeof onChatList === "function") {
      onChatList(list);
    }
  });

  socket.on("chat_history", (messages) => {
    if (typeof onChatHistory === "function") {
  ...
  socket.on("available_users", (list) => {
    if (typeof onAvailableUsers === "function") {
