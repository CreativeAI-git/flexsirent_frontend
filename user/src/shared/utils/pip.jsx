...
export const WebURL = import.meta.env.VITE_CANONICAL_URL 
  ? (import.meta.env.VITE_CANONICAL_URL.endsWith("/") ? import.meta.env.VITE_CANONICAL_URL : `${import.meta.env.VITE_CANONICAL_URL}/`)
  : "https://flexsirent.com/";
// Set token
export const setToken = (panel, token) => {
  const key = `${panel}_token`;
  if (key && typeof localStorage !== "undefined") localStorage.setItem(key, token);
};

// Get token
export const getToken = (panel) => {
  const key = `${panel}_token`;
  return (key && typeof localStorage !== "undefined") ? localStorage.getItem(key) : null;
};
// Set Permission
export const setPermissions = (permissons) => {
  ...
  const key = `subHostPermision`;
  if (key && typeof localStorage !== "undefined") localStorage.setItem(key, JSON.stringify(permissons));
};
...
export const getPermissions = () => {
  ...
  const permissons = localStorage.getItem(key)
  return permissons ? JSON.parse(localStorage.getItem(key)) : [];
};
// delete token
export const clearAuth = (panel) => {
  ...
  if (typeof localStorage !== "undefined") {
    ...
    if (panel == "host") {
      localStorage.removeItem("subHostPermision");
      localStorage.removeItem("isSubHost");
    }
  }
};
...
export const getCurrentPanel = () => {
  ...
  const list = ["host-business", "host", "guest-business", "guest"];
...
// Month View Format
export const pipViewMonth = (date) => {
  if (!date) return;
  return moment(date).format("MMMM");
};
export const pipViewMonthYear = (date) => {
  if (!date) return;
  return moment(date).format("MMMM YYYY");
};
// from-to View Format
export const pipFromTo = (date1, date2) => {
  if (!date1 || !date2) return;
  const formatted = `${moment(date1).format("D MMM")} - ${moment(date2).format("D MMM YYYY")}`;
...
export const getActivePanel = () => {
  ...
  for (let panel of PANEL_ORDER) {
    const token = localStorage.getItem(`${panel}_token`);
    if (token) return panel;
  }
  return null;
};

export const getActiveProfile = (panel) => {
  if (!panel) return {};
  if (typeof localStorage === "undefined") return {};

  if (panel === "guest" || panel === "host") {
    return JSON.parse(localStorage.getItem(`${panel}_profile`) || "{}");
  }

  if (panel === "guest-business") {
    return JSON.parse(localStorage.getItem("guestBusiness_profile") || "{}");
  }

  if (panel === "host-business") {
    return JSON.parse(localStorage.getItem("hostBusiness_profile") || "{}");
  }
...
export const getAnyActiveToken = () => {
  const roles = ["guest", "guest-business", "host", "host-business"];
...
export const apartmentData = [
  {
    id: 1,
    img: "https://app.flexsirent.com/assets/img/house_rent.png",
    ...
    features: [
      {
        icon: "https://app.flexsirent.com/assets/img/weui_location-outlined.svg",
        label: "Wi fi",
      },
      {
        icon: "https://app.flexsirent.com/assets/img/thunder_icon.png",
        label: "Bills Included",
      },
      {
        icon: "https://app.flexsirent.com/assets/img/wokspace_icon.png",
        label: "Workspace",
      },
    ],
    price: "850",
...
export const testimonialData = [
  {
    ...
    role: "Happy User",
    image:
      "https://app.flexsirent.com/assets/img/home/client_1.jpg",
    quote:
    ...
    quoteIcon:
      "https://app.flexsirent.com/assets/img/quote.png",
  },
  {
    id: 2,
    name: "Luise Aymar",
    role: "Happy User",
    image:
      "https://app.flexsirent.com/assets/img/home/client_2.jpg",
    quote:
      "My first time using the platform was a breeze – really polished and user friendly. One of their team even got in touch to guide me through the necessary steps. I highly recommend them!",
    quoteIcon:
      "https://app.flexsirent.com/assets/img/quote.png",
...
export const services = [
  {
    title: "Easy & Simple",
    description:
      "List your property in just a few minutes using our intuitive and hassle-free tools — no tech skills needed. Hosting made effortless.",
    icon: (
      <svg
        width="26"
        height="26"
