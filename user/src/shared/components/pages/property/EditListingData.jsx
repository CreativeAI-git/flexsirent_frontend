...
const EditListingData = ({ redirectURL, role = "", data = {} }) => {
  ...
  const initialValues = {
    ...
    min_stay_duration: data?.min_stay_duration || "",
    max_person: data?.max_person || "",
    monthly_rent: data?.monthly_rent || "",
    cleaning_fee: data?.cleaning_fee || "",
    cleaning_fee_type: data?.cleaning_fee_type || 0,
    monthly_rent_type: data?.monthly_rent_type || 0,
    bedrooms: data?.bedrooms || 0,
    bathrooms: data?.bathrooms || 0,
    beds: data?.beds || 0,
    square_foot: data?.square_foot || 0,
    videoFile: data?.video_url ? data?.video_url : null,
    house_rules: data?.house_rules?.split(",")?.map(Number) || [],
    amenities: data?.amenities?.split(",")?.map(Number) || [],
    ideal_for: data?.ideal_for?.split(",")?.map(Number) || [],
    safety_amenities: data?.safety_amenities?.split(",")?.map(Number) || [],
    available_from: moment(data?.available_from).format("YYYY-MM-DD") || null,
  };
  ...
  const handleAddListing = (values) => {
    ...
    if (deletedIds?.length > 0) {
      formdata.append("deleteImageIds", deletedIds.join(","))
    }
                  ...
                  <div className="ct_light_blue_outline py-4 ct_mt_40">
                    <div className="col-md-12">
                      <MediaUploader
                        ...
                        maxFiles={5 - existImage?.length}
                        deleteMemberImage={deleteMemberImage}
                        onDeleteImageIdsChange={(ids) => setDeletedIds(ids)}
