...
const UserInboxModal = ({ isViewModal, setIsViewModal, title = "" }) => {
  const dispatch = useDispatch();
  const [reply_message, setReplyMessage] = useState("");
  const { supportDetail } = useSelector((state) => state.host.inbox);
  ...
  const handleReply = () => {
    ...
    const callback = (res) => {
      if (res?.success) {
        ...
        if (supportDetail?.activeTab == "guestQueries") {
              ...
              <form className="">
                <div className="form-group mb-4">
                  <label className="mb-2 ct_fw_600">
                    {supportDetail?.activeTab == "guestQueries"
                  ...
                  supportDetail?.reply_message?.map((item, ind) => (
                    <div className="form-group mb-4">
                      <label className="mb-2 ct_fw_600 text-capitalize">
                        {item?.role == 'host' ? 'Your' : item?.role || "#N/A"} Reply ({pipViewDateTime(item?.created_at)})
                      </label>
                      <p className="mb-0 ct_para_scroll ct_custom_scroll">
                        {item?.message || "#N/A"}
                      </p>
