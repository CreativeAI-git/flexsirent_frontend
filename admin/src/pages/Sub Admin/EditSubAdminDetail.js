...
const EditSubAdminDetail = () => {
  ...
  const initialValues = {
    full_name: subAdminDetails?.full_name || "",
    email: subAdminDetails?.email || "",
    mobile: subAdminDetails?.mobile || "",
    permission: subAdminDetails?.permission?.map((item) => item?.type) || [],
  };
  ...
  const handleAddSubAdmin = (values) => {
    ...
    dispatch(
      updateSubAdmin({
        payload: { ...values, sub_admin_id: subAdminDetails?.admin_id },
        callback,
      })
    );
  };
  ...
  return (
    <PanelLayout>
      <SubHeader label="Edit Sub Admin Details" />
...
export default EditSubAdminDetail;
