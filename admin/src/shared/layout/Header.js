import { useEffect } from "react";
import { getProfile } from "../../utills/pip";
import { pageRoutes } from "../../routes/PageRoutes";
import { useDispatch, useSelector } from "react-redux";
import { useLocation, useNavigate } from "react-router";
import ImageWithPreview from "../components/image preview/imageWithPreview";
import {
  myProfile,
  fetchAdminNotifications,
} from "../../redux/actions/authAction";
import {
  toggleSideBarView,
  markNotificationsRead,
} from "../../redux/reducers/authReducers";

const Header = () => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const { pathname } = useLocation();
  const role = localStorage.getItem("adminRole");
  const profileData = getProfile("admin");
  const { unreadNotificationCount } = useSelector((state) => state.authReducers);

  const handleLogoutAdmin = () => {
    localStorage.removeItem("admin_token");
    localStorage.removeItem("admin_profile");
    window.location.href = `/admin${pageRoutes.login}`;
  };

  useEffect(() => {
    const profile = getProfile("admin");
    if (!Object.keys(profile).length) {
      dispatch(myProfile());
    }
  }, [dispatch]);

  useEffect(() => {
    if (pathname !== pageRoutes.notification) {
      dispatch(fetchAdminNotifications());
    }
  }, [dispatch, pathname]);

  return (
    <header>
      <div className="d-flex align-items-center gap-2">
        <i
          className="fa-solid fa-bars me-3 ct_fs_18 ct_menu_bar ct_cursor_pointer"
          onClick={() => dispatch(toggleSideBarView(true))}
        ></i>
      </div>
      <div className="d-flex align-items-center gap-3">
        <div className="ct_border_right_1">
          <a
            href="#"
            onClick={(e) => {
              e.preventDefault();
              dispatch(markNotificationsRead());
              navigate(pageRoutes.notification);
            }}
            className="ct_notification_icon"
          >
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M12 6.43994V9.76994"
                stroke="#292D32"
                strokeWidth="1.5"
                strokeMiterlimit="10"
                strokeLinecap="round"
              />
              <path
                d="M12.0199 2C8.3399 2 5.3599 4.98 5.3599 8.66V10.76C5.3599 11.44 5.0799 12.46 4.7299 13.04L3.4599 15.16C2.6799 16.47 3.2199 17.93 4.6599 18.41C9.4399 20 14.6099 20 19.3899 18.41C20.7399 17.96 21.3199 16.38 20.5899 15.16L19.3199 13.04C18.9699 12.46 18.6899 11.43 18.6899 10.76V8.66C18.6799 5 15.6799 2 12.0199 2Z"
                stroke="#292D32"
                strokeWidth="1.5"
                strokeMiterlimit="10"
                strokeLinecap="round"
              />
              <path
                d="M15.3299 18.8201C15.3299 20.6501 13.8299 22.1501 11.9999 22.1501C11.0899 22.1501 10.2499 21.7701 9.64992 21.1701C9.04992 20.5701 8.66992 19.7301 8.66992 18.8201"
                stroke="#292D32"
                strokeWidth="1.5"
                strokeMiterlimit="10"
              />
            </svg>
           {unreadNotificationCount > 0 ?  <span></span> :null}
          </a>
        </div>
        <div className="ct_right_dropdown dropdown">
          <button
            id="dropdownMenuButton"
            data-bs-toggle="dropdown"
            aria-expanded="false"
          >
            <div className="d-flex align-items-center gap-2">
              <ImageWithPreview
                isPreview={false}
                data="ct_img_30"
                image={
                  profileData?.image || "assets/img/user.png"
                }
              />
              <div className="text-start">
                <p className="ct_fs_16 mb-0 ct_fw_500 d-sm-block d-none">
                  {profileData?.full_name ?? ""}
                </p>
                <small className="ct_text_939393">{role}</small>
              </div>
            </div>
            <i className="fa-solid fa-angle-down ms-auto"></i>
          </button>
          <ul className="dropdown-menu" aria-labelledby="dropdownMenuButton1">
            <li>
              <a
                className="dropdown-item"
                href="#"
                onClick={(e) => {
                  e.preventDefault();
                  navigate(pageRoutes.myProfile);
                }}
              >
                My Profile
              </a>
            </li>
            <li>
              <a
                className="dropdown-item"
                href="#"
                onClick={(e) => {
                  e.preventDefault();
                  navigate(pageRoutes.changePassword);
                }}
              >
                Change Password
              </a>
            </li>
            <li>
              <a
                className="dropdown-item"
                href="#"
                onClick={(e) => {
                  e.preventDefault();
                  handleLogoutAdmin();
                }}
              >
                Log Out
              </a>
            </li>
          </ul>
        </div>
      </div>
    </header>
  );
};

export default Header;
